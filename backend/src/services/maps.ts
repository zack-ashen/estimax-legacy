...
import { Location } from "../models/sub-schema/locationArea";
...
export const parseLocationData = async (
  locationData: PlaceAutocompleteResult
): Promise<Omit<Location, "unit">> => {
  ...
  if (!res.geometry) throw new Error("No geometry found");
  ...
  if (!res.address_components) throw new Error("No address components found");
    ...
    if (
      ...
      types.some(
        (type) =>
          ...
          type === AddressType.administrative_area_level_3 ||
          type === AddressType.sublocality
      )
    ) {
...
export const parseLocationAreaData = async (
  locationAreaData: PlaceAutocompleteResult
) => {
  const locationInfo = await googleMapsClient.placeDetails({
    params: {
      key: process.env.GOOGLE_MAPS_API!,
